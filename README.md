...
| `tools/ci/publish_gitlab.py` | реестр пакетов и Release — единственное, что знает про API GitLab |
...
**GitHub — зеркало и второй CI.** Там проверяются словарь, версии манифестов,
схемы общих артефактов, алгоритм публикации и юнит-сторона расширения VS Code.
Сторона IDEA (включая снимок `rules.json`, который тянет платформу),
интеграционный тест VS Code, упаковка и релиз там **не** запускаются — зелёный
статус GitHub означает ровно перечисленное, а не «всё проверено». Фильтр тегов
там шире (`v*`); релизным тег делает проверка `contract:version`, идущая на
обоих хостингах.

**Релиз выпускает только GitLab** — по защищённому тегу `v<версия>`; защита
