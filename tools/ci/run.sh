...
run_step() {
    ...
    case $step in
        ...
        test:publish)
            ...
            #
            # Байт-код не пишется намеренно. Кэш `__pycache__` считает исходник
            # неизменным при совпадении mtime (секундная точность) и размера —
            # то есть правка одинаковой длины, сделанная в ту же секунду,
            # исполняется старым байт-кодом. На мутационных прогонах это даёт
            # ложный результат в обе стороны.
            PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s tools/ci -p test_publish_gitlab.py
            ;;
        test:idea)
