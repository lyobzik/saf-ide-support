...
| `findusages/SmartAppKeywordClassUsages` | `CustomUsageSearcher`: **дополняет** поиск использований класса в Python вхождениями его слова в DSL; handler здесь нельзя — он отнял бы поиск у плагина Python |
...
| `core/classUsages.ts` | Поиск использований класса Python: класс по объявлению в диапазоне, слова по правой части регистрации (порт `SmartAppKeywordClassUsagesSearcher`) |
...
| `vscode/providers.ts`, `vscode/positions.ts` | Провайдеры VS Code и трансляция смещений в `Position` (и обратно — для объявлений, пришедших от Python-расширения) |
...
- **Направление «каретка в Python» — платформенное, не наше.** Без плагина
  ...
  Проверяется вручную при приёмке и описано в README. В IDEA работает в двух
  точках: строка регистрации (свой `FindUsagesHandler`) и класс из правой части
  регистрации (`CustomUsageSearcher`, который **дополняет** чужой результат).
  В VS Code — только от класса: отдельный `ReferenceProvider` для `python`
  (VS Code склеивает ответы всех провайдеров), объявление символа узнаётся у
  Python-расширения через `vscode.executeDefinitionProvider`, без него остаётся
  каретка на имени в `class X`. Отсюда же `onLanguage:python` в
  `activationEvents`: провайдер регистрируется при активации, и без этого
  события поиск от класса молчал бы до первого открытого JSON. Тесты доходят до
  последнего стыка и останавливаются: в IDEA элемент подставной, в VS Code ответ
  Python-расширения подменён в фейковом `vscode`, а smoke в настоящем редакторе
  проверяет только запасной путь — Python-расширения в тестовом VS Code нет.
- **Класс узнаётся по объявлению, а не по типу элемента.** PSI Python плагину
  недоступно, поэтому «под кареткой класс» проверяется своим же сканером: в
  диапазон элемента обязано попасть имя объявленного в этом файле класса. Одного
  совпадения имени мало — функция или переменная, названная как
  зарегистрированный класс, получила бы чужие вхождения. Цель проходит и фильтр
  исключённых каталогов: класс из `venv` приложению не принадлежит ровно так же,
  как JSON-файл оттуда. Правило одно с обеих сторон; в VS Code диапазоном
  бывает и объявление целиком, поэтому берётся первый по тексту класс — сам
  объявленный, а не вложенный в него.
...
отказ переименования и поиск от имени на строке регистрации в `.py` (есть
только в IDEA). Поиск от класса в `.py` есть с обеих сторон, но в корпус тоже
не пишется: каретку в `.py` в тестовой платформе IDEA не поставить.
