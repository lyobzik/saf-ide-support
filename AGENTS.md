...
- **Направление «каретка в Python» — платформенное, не наше.** Без плагина
  ...
  Проверяется вручную при приёмке и описано в README. Работает в двух точках:
  строка регистрации (свой `FindUsagesHandler`) и класс из правой части
  регистрации (`CustomUsageSearcher`, который **дополняет** чужой результат).
  Тесты доходят до последнего стыка и останавливаются: элемент подставной, а
  «платформа отдала нам `PyClass`» проверить нечем.
