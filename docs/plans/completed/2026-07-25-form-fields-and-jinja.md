...
> - `SmartAppFieldRef.targetFormOf` получил опциональный параметр `fileKind`:
> автодополнение работает на in-memory копии, теряющей реальный путь, поэтому
> `SmartAppFiles.kindOf(literal.containingFile)` в копии возвращает null. Completion
> передаёт `fileKind`, определённый по `parameters.originalFile`.
> - В `addFieldVariants` добавлен явный `result.withPrefixMatcher(fieldPrefix)`,
> иначе платформа фильтровала варианты по всему `main_form.` префиксу и имена полей
> не совпадали (lookup пустой).
> - F2 `testFindUsagesProviderRejectsNestedProperty` переписан: поле формы теперь
> first-class (допускает Find Usages); добавлен отдельный тест, что произвольное
> вложенное свойство (`type`) по-прежнему отвергается.
>
> **Корректирующий фикс (4 P1 + усиление тестов):** после первой реализации
> ревью вскрыло функциональные дефекты, исправленные отдельным `fix:`-коммитом:
> - **Лексер работает по decoded-тексту.** Введён `JsonStringLiteralDecoder`:
> raw-содержимое литерала декодируется (раскрытие `\"`, `\\`, `\n`, `\uXXXX`, …)
> до лексинга, и лексер видит корректный Jinja с двойными строками; диапазоны
> токенов транслируются обратно в raw через карту `decoded->raw` смещений. Без
> этого raw JSON escape поглощал хвост выражения.
> - **`findCloseDelim` пропускает Jinja-строки** — `{{ '}}' }}` и
> `{% if x == '%}' %}` более не закрываются преждевременно.
> - **Семантика полей ограничена интерполяцией `{{ }}`.** Введены
> `INTERP_OPEN/CLOSE` и `STATEMENT_OPEN/CLOSE`; ссылки на поля и WARNING
> создаются только внутри `{{ … }}`. Statement-тег `{% set x = main_form.unknown %}`
> более не даёт ни ссылки, ни WARNING. Contributor дополнительно проверяет
> `property.value === literal`, чтобы не навешивать ссылки на JSON-ключи.
> - **Имя фильтра помечается `FILTER_NAME`** (новый тип токена) и красится
> `JINJA_FILTER`, как и оператор `|` — согласно контракту плана.
> - **Completion:** удалён diagnostic `error(...)` (падал в `{% %}` и до первого
> `{{`); добавлена проверка, что каретка внутри актуальной открытой интерполяции
...
> malformed-входах.
> - **Completion работает и в незакрытой интерполяции** (`{{ main_form.<caret>`
> в момент набора): контекст достраивается синтетическим `}}`; каретка внутри
> строки или statement-тега по-прежнему отвергается лексером.
>
> **Корректирующий цикл 3 (границы scope и completion):**
> - **Цепочки вне scope не получают семантику поля.** `fieldCandidates` проверяет
> границы триплета: `main_form` в позиции чужого поля (`variables.main_form.x`)
> и подобъекты (`main_form.x.y`) не дают ни ссылки, ни WARNING; операндная
> позиция (`x + main_form.name`) по-прежнему резолвится.
> - **Completion после пробела за точкой** (`{{ main_form. <caret> }}`): префикс
> нормализуется (пробелы отбрасываются), поля предлагаются.
> - **Контракт имён полей:** completion предлагает только identifier-имена (в
> терминах лексера). Поля с пунктуацией (`contact.email`) из Jinja недоступны;
> синтаксис доступа к ним — отдельное проектирование (см. Future work).
> - **Completion не активируется в позиции цепочки** (`{{ main_form.x.<caret> }}`)
> и после не-identifier начала (`main_form.2<caret>`): хвост между `main_form.`
> и кареткой валидируется полным матчем, иначе предлагались бы заведомо битые
> варианты.
> - Все продуктовые шаги 1–7 выполнены; **120 тестов зелёных** (62 базовых
> + 19 лексер + 39 полей), 0 падений.
...
До реализации плана **любое** строковое значение, содержащее `{{` или `{%}`,
полностью пропускалось (`SmartAppReferenceContributor.isJinja` → ранний
`return EMPTY`; дублирующая проверка в аннотаторе). Такие значения не резолвились и
не подсвечивались как неразрешённые — это сознательная политика v1, чтобы динамические
...
- **`object FormFieldRefDescriptor : KeyDescriptor<FormFieldRef>`** —
  `save`/`read`/`getHashCode`/`isEqual`. Сериализация — две length-prefixed
...
- **Защита от частичного/битого JSON:** guard `JsonPsi.hasError(jsonFile)` (из Плана 1)
  сразу после safe-cast `psiFile as? JsonFile` и **до** извлечения `topLevelValue` →
  `emptyMap()`. `try/catch → emptyMap()` сохраняется как страховка.
- **`getVersion()` = `1 + SERIALIZATION_VERSION + CONTENT_VERSION`**
  (`CONTENT_VERSION` — версия алгоритма, отдельно от формата сериализации; по
  образцу `SmartAppDefinitionIndex`). При смене externalizer'а версия поднимется
  автоматически через `SERIALIZATION_VERSION`.
...
- **Ключ — имя формы** через `EnumeratorStringDescriptor.INSTANCE` (однозначная
  строковая сериализация, без коллизий с `:`).
...
- **`getVersion()` = `1 + CONTENT_VERSION`** (своя `CONTENT_VERSION` — версия
  алгоритма, не заимствованная у externalizer'а форм).
...
**Не полноценный Jinja-парсер, а детерминированный лексер.** На входе —
**decoded**-текст literal (raw JSON-содержимое без кавычек с раскрытыми escape через
`JsonStringLiteralDecoder`, см. шапку статуса): это позволяет корректно разбирать
Jinja с двойными строками в JSON-представлении
(`"{{ x | default(\"y\") }}"`). На выходе — упорядоченный список токенов с
`(startOffset, endOffset)` относительно decoded-текста; перевод обратно в raw —
через карту `decoded->raw` смещений.

**Контракт:**
- **Парные разделители учитываются семантически, с пропуском Jinja-строк.**
  Сканирование decoded-текста слева направо: `{{ … }}` и `{% … %}` — парные;
  закрывающий разделитель ищется **мимо строковых литералов** Jinja (`{{ '}}' }}`
  корректно). **Непарные** (нет закрывающего `}}`/`%}` до конца literal) — весь
  остаток помечается одним `TEXT`-токеном, **ошибкой/ссылкой не становится**.
- **Разделение интерполяции и statement-тегов:** `{{`/`}}` дают
  `INTERP_OPEN`/`INTERP_CLOSE`, `{%`/`%}` — `STATEMENT_OPEN`/`STATEMENT_CLOSE`.
  Семантика полей (ссылки, WARNING, completion) создаётся **только** внутри
...
- **Внутри `{{ … }}` токены** (приоритет, жадность слева направо):
  1. `VAR` — `\b(?:main_form)\b` либо `\b[A-Za-z_][A-Za-z0-9_]*\b` после
     `{{`/`.`/оператора → идентификатор (после `|` — `FILTER_NAME`, см. п.3).
     **Семантическая ссылка создаётся
  ...
  4. `STRING` — `"…"` или `'…'` с учётом escape (`\"`, `\\`, …). **Внутри строки
     ...
     не `VAR`).
  5. Разделители `{{`/`}}` → `INTERP_OPEN`/`INTERP_CLOSE`, `{%`/`%}` →
     `STATEMENT_OPEN`/`STATEMENT_CLOSE`.
  6. `TEXT` — всё прочее (внутри `{{ }}` и снаружи).
- **`{% … %}`** — **только лексическая подсветка** (`STATEMENT_*` + внутренние
  `VAR`/`STRING`/`TEXT`); **семантических ссылок и WARNING не создаётся**
  (см. `fieldCandidates()` — он обходит только интерполяции).
...
- **Версионирование индексов** (политика из `AGENTS.md`): `SmartAppFormFieldIndex`
  наследует версию от externalizer'а (`1 + SERIALIZATION_VERSION + CONTENT_VERSION=1`);
  `SmartAppFormFieldNameIndex` имеет свою версию `getVersion() = 2`
  (`1 + CONTENT_VERSION=1`). При смене формата сериализации соответствующая версия
  поднимается.
...
- Синтаксис доступа к полям с не-identifier именами (`contact.email`,
  `field:name`) — сейчас они отфильтрованы из completion; нужен отдельный дизайн
  (например, `main_form['contact.email']`).
