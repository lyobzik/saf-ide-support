...
describe("манифест расширения", () => {
  ...
  it("активируется по языку JSON — этого достаточно для функций DSL", () => {
    expect(manifest.activationEvents).toContain("onLanguage:json");
  });

  it("активируется и по языку Python — иначе поиск от класса молчит до первого JSON", () => {
    // Провайдер для `.py` регистрируется при активации. Открыл пользователь
    // сначала Python-файл — без этого события расширение ещё не запущено, и
    // поиск использований класса не находит строк DSL без всякой причины.
    expect(manifest.activationEvents).toContain("onLanguage:python");
  });
});
