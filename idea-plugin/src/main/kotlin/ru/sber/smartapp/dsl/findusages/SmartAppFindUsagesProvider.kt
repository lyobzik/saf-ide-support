 ...
 * имён полей внутри выражений Jinja (`{{ … }}` и `{% … %}`). Элементы, не
 * являющиеся ни тем, ни другим, отклоняются — их обрабатывает встроенный
 * JSON-провайдер.
 *
 * Вхождения самого `main_form` использованиями формы не считаются: имени формы
 * в тексте нет, платформа ищет по слову-имени и до них не доходит.
 */
